...
def calculate_added_money_week_sku_type(promo_flag, sku, week, type_):
    return sum([type_weights[type_] * sku_weights[sku] * week_from_start_weights[t] * week_cycle(promo_flag, sku, week, type_, -t) for t in LOOK_BACK])
...
def calculate_schedule(args):
    ...
    # Целевая
    model.OBJ = pyo.Objective(expr=sum(calculate_added_money_week_sku_type(model.promo, s, w, t)  for w in WEEKS for s in SKUS for t in TYPES), sense=pyo.maximize)
